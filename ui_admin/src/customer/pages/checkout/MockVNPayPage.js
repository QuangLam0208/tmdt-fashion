...
const MockVNPayPage = () => {
  ...
  const handleSuccess = async () => {
    ...
    try {
      ...
      window.location.href = `/checkout/payment-result?paymentStatus=success&orderId=${orderId}`;
    } catch (error) {
  ...
  const handleFail = () => {
    window.location.href = `/checkout/payment-result?paymentStatus=failed&orderId=${orderId}`;
  };

  return (
    <div style={{ backgroundColor: '#003a63', minHeight: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', padding: 20 }}>
      <Card style={{ width: 450, textAlign: 'center', borderRadius: 16, boxShadow: '0 8px 24px rgba(0,0,0,0.2)' }}>
        <img src="https://upload.wikimedia.org/wikipedia/vi/1/1a/VNPAY_QR_logo.png" alt="VNPay" style={{ width: 90, marginBottom: 16 }} />
        <Title level={4} style={{ color: '#003a63', marginTop: 0 }}>CỔNG THANH TOÁN (MOCK)</Title>
        ...
        <Space direction="vertical" style={{ width: '100%' }} size="middle">
          <Button
            ...
            loading={loading}
            style={{ background: '#003a63', borderColor: '#003a63', height: 50, fontWeight: 'bold' }}
          >
