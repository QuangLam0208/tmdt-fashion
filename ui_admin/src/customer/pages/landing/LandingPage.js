...
const LandingPage = () => {
  ...
  const handleCategoryClick = (id) => {
    if (id) navigate(`/shop?categoryId=${id}`);
  };

  return (
    <div>
      {/* ── BannerSlider — tải ngay vì ở đầu trang ────────────────────── */}
      <BannerSlider />
              ...
              return (
                <div key={parentId || index} style={{ marginBottom: 40 }}>
                  <div style={{ display:'flex', alignItems:'center', marginBottom: 20 }}>
                    ...
                    <div style={{ flex:1, height:1, background:'#eaeaea', marginLeft:16 }} />
                    <span
                      onClick={() => handleCategoryClick(parentId)}
                      style={{ marginLeft:16, color:'#1677ff', cursor:'pointer', fontSize:'0.9rem', fontWeight:600, whiteSpace:'nowrap' }}
                    >
                      Xem tất cả &gt;
                    </span>
                  </div>
                  ...
                  <div className="landing-categories">
                    {subCats.length > 0 ? (
                      subCats.map((sub, i) => {
                        ...
                        return (
                          <div key={subId || `sub-${i}`} onClick={() => handleCategoryClick(subId)} style={{ cursor:'pointer' }}>
